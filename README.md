...
| <a name="input_filesystems"></a> [filesystems](#input\_filesystems) | Filesystems to attach to instances | <pre>map(object({<br/>    filesystem_id = string<br/>    device_name   = optional(string)<br/>    mode          = optional(string, "READ_WRITE")<br/>  }))</pre> | `{}` | no |
| <a name="input_folder_id"></a> [folder\_id](#input\_folder\_id) | Folder ID where the instance group will be created | `string` | `null` | no |
...
| <a name="input_gpus"></a> [gpus](#input\_gpus) | Number of GPU devices for the instance | `number` | `0` | no |
...
| <a name="input_metadata_options"></a> [metadata\_options](#input\_metadata\_options) | Metadata options configuration for accessing instance metadata | <pre>object({<br/>    aws_v1_http_endpoint = optional(number, 1)<br/>    aws_v1_http_token    = optional(number, 1)<br/>    gce_http_endpoint    = optional(number, 1)<br/>    gce_http_token       = optional(number, 1)<br/>  })</pre> | `{}` | no |
...
| <a name="input_network_interface_dns_records"></a> [network\_interface\_dns\_records](#input\_network\_interface\_dns\_records) | DNS records configuration for network interface | <pre>list(object({<br/>    dns_zone_id = optional(string)<br/>    fqdn        = string<br/>    ptr         = optional(bool, false)<br/>    ttl         = optional(number, 300)<br/>  }))</pre> | `[]` | no |
| <a name="input_network_interface_ipv6_dns_records"></a> [network\_interface\_ipv6\_dns\_records](#input\_network\_interface\_ipv6\_dns\_records) | IPv6 DNS records configuration for network interface | <pre>list(object({<br/>    dns_zone_id = optional(string)<br/>    fqdn        = string<br/>    ptr         = optional(bool, false)<br/>    ttl         = optional(number, 300)<br/>  }))</pre> | `[]` | no |
| <a name="input_network_interface_nat_dns_records"></a> [network\_interface\_nat\_dns\_records](#input\_network\_interface\_nat\_dns\_records) | NAT DNS records configuration for network interface | <pre>list(object({<br/>    dns_zone_id = optional(string)<br/>    fqdn        = string<br/>    ptr         = optional(bool, false)<br/>    ttl         = optional(number, 300)<br/>  }))</pre> | `[]` | no |
